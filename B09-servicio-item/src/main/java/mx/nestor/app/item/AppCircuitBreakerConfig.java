	...
	public Customizer<Resilience4JCircuitBreakerFactory> defaultCuztomizer() {
		...
		return factory -> factory.configureDefault(id -> {
			return new Resilience4JConfigBuilder(id)
					.circuitBreakerConfig(CircuitBreakerConfig.custom().slidingWindowSize(10) // (muestreo estadistico)
																								// Número de peticiones
																								// a fallar
							.failureRateThreshold(50) // umbral de porcentaje a falla (contabilizar las fallas, cambio
														// de estados)
							.waitDurationInOpenState(Duration.ofSeconds(10L))// Tiempo donde se pertenece el estado
																				// abierto (cortocircuito)
							.permittedNumberOfCallsInHalfOpenState(5) // cambio del estado abierto a semi abierto
